...
void conjugate_gradient(skepu::Matrix<T> BLAS_CONST& A, skepu::Vector<T> BLAS_CONST& b, skepu::Vector<T> &x)
{
	...
	skepu::Vector<T> p(N, "p"), r(N, "r"), Ap(N, "Ap");
	...
	for (size_t k = 0; k < iters; ++k)
	{
		...
		auto tmp = skepu::blas::dot(N, p, 1, Ap, 1); // tmp := p * Ap = p * A * p
		auto alpha = rTr / tmp;

		// Update x
		std::cout << "Size of x: " << x.size() << " and r: " << r.size() << "\n";
		...
		// Compute beta
		auto rTr_new = skepu::blas::dot(N, r, 1, r, 1); // rTr_new := r * r
		auto beta = rTr_new / rTr;
...
int main(int argc, char *argv[])
{
	...
	skepu::Vector<T> b(N, "b");
	skepu::Matrix<T> A(N, N, "A");
	skepu::Vector<T> x(N, "x", 0);
//	b = {1, 2};
//	A = {4, 1, 1, 3};
	b.randomize(0, 10); // PRNG
	A.randomizeReal(); // PRNG

//	skepu::io::cout << "~~~ Conjugate gradient ~~~\n";
//	skepu::io::cout << "A: " << A << "\n";
//	skepu::io::cout << "b: " << b << "\n";
