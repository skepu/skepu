...
int main(int argc, char *argv[])
{
	...
	conv2_m.setEdgeMode(skepu::Edge::Cyclic);

	skepu::Matrix<float> m(size, size, 10);
	skepu::Matrix<float> filter(3, 3, .25);
	skepu::Matrix<float> res_f(size, size);
	skepu::Matrix<int> res_int(size, size);


	conv2_m(res_f, res_int, m, filter);
	skepu::external(
		skepu::read(res_f, res_int),
		[&]
		{
			std::cout << "res (float): " << res_f << "\n"
				<< "res (int): " << res_int << "\n";
		});
