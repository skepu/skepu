...
__kernel void {{KERNEL_NAME}}(
	...
	size_t skepu_n,
	size_t skepu_base,
	__local {{REDUCE_RESULT_TYPE}}* skepu_sdata
)
{
	...
	while (skepu_i < skepu_n)
	{
		...
		{{CONTAINER_PROXIE_INNER}}
#if !{{USE_MULTIRETURN}}
		{{MAP_RESULT_TYPE}} tempMap = {{FUNCTION_NAME_MAP}}({{MAP_ARGS}});
...
__kernel void {{KERNEL_NAME}}_ReduceOnly(
//	__global {{REDUCE_RESULT_TYPE}}* skepu_input,
//	__global {{REDUCE_RESULT_TYPE}}* skepu_output,
	{{KERNEL_PARAMS_REDUCE_ONLY}}
	size_t skepu_n,
	__local {{REDUCE_RESULT_TYPE}}* skepu_sdata
)
{
	...
	if (skepu_i < skepu_n)
	{
		skepu_result = skepu_output[skepu_i];
		skepu_i += skepu_gridSize;
	}

	while (skepu_i < skepu_n)
	{
		skepu_result = {{FUNCTION_NAME_REDUCE}}(skepu_result, skepu_output[skepu_i]);
		skepu_i += skepu_gridSize;
	}
...
public:
	...
	)
	{
		...
		skepu::backend::cl_helpers::setKernelArgs(skepu_kernel, {{KERNEL_ARGS}} {{SIZE_ARGS}} {{STRIDE_ARGS}} skepu_n, skepu_base);
	...
	//	skepu::backend::DeviceMemPointer_CL<{{REDUCE_RESULT_CPU}}> *skepu_input, skepu::backend::DeviceMemPointer_CL<{{REDUCE_RESULT_CPU}}> *skepu_output,
		{{HOST_KERNEL_PARAMS_REDUCE_ONLY}}
		size_t skepu_n, size_t skepu_sharedMemSize
	)
	{
		cl_kernel skepu_kernel = kernels(skepu_deviceID, KERNEL_REDUCE);
		skepu::backend::cl_helpers::setKernelArgs(skepu_kernel,
		//	skepu_input->getDeviceDataPointer(), skepu_output->getDeviceDataPointer(),
			{{KERNEL_ARGS_REDUCE_ONLY}} skepu_n);
		clSetKernelArg(skepu_kernel, /*3*/ {{KERNEL_ARG_COUNT_REDUCE_ONLY}}, skepu_sharedMemSize, NULL);
		cl_int skepu_err = clEnqueueNDRangeKernel(skepu::backend::Environment<int>::getInstance()->m_devices_CL.at(skepu_deviceID)->getQueue(), skepu_kernel, 1, NULL, &skepu_globalSize, &skepu_localSize, 0, NULL, NULL);
...
std::string createMapReduceKernelProgram_CL(SkeletonInstance &instance, UserFunction &mapFunc, UserFunction &reduceFunc, std::string dir)
{
	...
	std::stringstream SSKernelParamListRO, SSHostKernelParamListRO, SSKernelArgsRO;
	...
	handleOutputs_reduceOnly_CL(mapFunc, SSHostKernelParamListRO, SSKernelParamListRO, SSKernelArgsRO, true);
	...
	std::stringstream SSKernelArgCount, SSKernelArgCountRO;
	SSKernelArgCount << mapFunc.numKernelArgsCL() + 2 + indexInfo.sizeParamCount + stride_counter;
	SSKernelArgCountRO << 1 + std::max<size_t>(1, mapFunc.multipleReturnTypes.size());
	...
	FSOutFile << templateString(Constructor,
	{
		...
		{"{{KERNEL_ARGS_REDUCE_ONLY}}",            SSKernelArgsRO.str()},
		{"{{KERNEL_PARAMS_REDUCE_ONLY}}",          SSKernelParamListRO.str()},
		{"{{HOST_KERNEL_PARAMS_REDUCE_ONLY}}",     SSHostKernelParamListRO.str()},
		{"{{KERNEL_ARG_COUNT_REDUCE_ONLY}}",       SSKernelArgCountRO.str()}
	});
