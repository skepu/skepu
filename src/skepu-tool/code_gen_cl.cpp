...
IndexCodeGen indexInitHelper_CL(UserFunction &uf)
{
	...
	if (uf.indexed1D || uf.indexed2D || uf.indexed3D || uf.indexed4D || uf.hasMatRowOrCol)
	{
	...
	else if (uf.indexed2D || uf.hasMatRowOrCol)
	{
		res.dim = 2;
...
{ return r.data[i * r.stride1 * r.stride2 * r.stride3 + j * r.stride2 * r.stride3 + k * r.stride3 + l]; }
	...
	else
	{
		switch (dim)
		{
		case 1: retval = OpenCLPool1DTemplate; break;
		case 2: retval = OpenCLPool2DTemplate; break;
		case 3: retval = OpenCLPool3DTemplate; break;
...
RND_STATE_T skepu_random_get(__global skepu_random *prng)
{
	...
	return prng->m_state >> 17;
}

RND_NORMALIZED_T skepu_random_get_normalized(__global skepu_random *prng)
{
	return (RND_NORMALIZED_T)skepu_random_get(prng) / RND_NORMAL_DIV;
}
)~~~";
...
)
{
	...
	for (UserFunction::RandomAccessParam& param : func.anyContainerParams)
	{
		...
		switch (param.containerType)
		{
			...
			case ContainerType::MatRow:
				...
				SSProxyInitializerInner << param.TypeNameOpenCL() << " " << param.name << " = { .data = (" << name << " + (skepu_i / skepu_w2) * skepu_cols_" << param.name << "), .cols = skepu_cols_" << param.name << " };\n";
				break;
			
			case ContainerType::MatCol:
				SSKernelParamList << "__global " << param.innerTypeNameOpenCL() << " *" << name << ", size_t skepu_rows_" << param.name << ", size_t skepu_cols_" << param.name << ", ";
				SSKernelArgs << "std::get<1>(" << name << ")->getDeviceDataPointer(), std::get<0>(" << name << ")->total_rows(), std::get<0>(" << name << ")->total_cols(), ";
				SSProxyInitializerInner << param.TypeNameOpenCL() << " " << param.name << " = { .data = (" << name << " + (skepu_i % skepu_w2)), .rows = skepu_rows_" << param.name << ", .cols = skepu_cols_" << param.name << " };\n";
				break;
	...
	{
		if (!first) { SSMapFuncArgs << ", "; }
		SSKernelParamList << param.typeNameOpenCL() << " " << param.name << ", ";
...
std::string handleOutputs_CL(UserFunction &func, std::stringstream &SSHostKernelParamList, std::stringstream &SSKernelParamList, std::stringstream &SSKernelArgs, bool strided, std::string index)
{
	...
	else
	{
		...
		{
			std::string outputTypeCL = outputType;
			replaceTextInString(outputTypeCL, "struct ", "");
			outputTypeCL = transformToCXXIdentifier(outputTypeCL);
			std::stringstream strideinc;
			if (strided) strideinc << " * skepu_stride_" << outCtr;
			SSKernelParamList << "__global " << outputTypeCL << "* skepu_output_" << outCtr << ", ";
...
std::string handleOutputs_reduceOnly_CL(UserFunction &func, std::stringstream &SSHostKernelParamList, std::stringstream &SSKernelParamList, std::stringstream &SSKernelArgs, bool strided, std::string index)
{
	if (func.multipleReturnTypes.size() == 0)
	{
		SSKernelParamList << " __global " << func.rawReturnTypeName << "* skepu_output, ";
...
std::string generateUserFunctionCode_CL(UserFunction &Func)
{
	...
	if (UserFunction::RegionParam *param = Func.regionParam)
	{
		...
		else if (param->containerType == ContainerType::Pool1D)
			SSFuncParamList << "skepu_pool1d_" << transformToCXXIdentifier(param->resolvedTypeName) << " " << param->name;
		else if (param->containerType == ContainerType::Pool2D)
			SSFuncParamList << "skepu_pool2d_" << transformToCXXIdentifier(param->resolvedTypeName) << " " << param->name;
		else if (param->containerType == ContainerType::Pool3D)
			SSFuncParamList << "skepu_pool3d_" << transformToCXXIdentifier(param->resolvedTypeName) << " " << param->name;
		else if (param->containerType == ContainerType::Pool4D)
			SSFuncParamList << "skepu_pool4d_" << transformToCXXIdentifier(param->resolvedTypeName) << " " << param->name;
		first = false;
	}
...
std::string generateOpenCLMultipleReturn(UserFunction &UF)
{
	if (UF.multipleReturnTypes.size() > 0)
	{
		...
		{
			replaceTextInString(type, "struct ", "");
			type = transformToCXXIdentifier(type);
