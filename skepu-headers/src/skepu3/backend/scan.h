...
namespace skepu
{
	namespace backend
	{
		...
		public:
			...
			Scan(std::string label, CUDAScan scan, CUDAScanUpdate update, CUDAScanAdd add)
			: SkeletonBase(label), m_cuda_scan_kernel(scan), m_cuda_scan_update_kernel(update), m_cuda_scan_add_kernel(add)
			{
#ifdef SKEPU_OPENCL
				CLKernel::initialize();
#endif
