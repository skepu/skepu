...
template <typename T>
		...
		void Environment<T>::init_CL()
		{
			if (getDataTypeCL<T>() == "int") // to ensure that only we create devices when using Environment<int> and rest we just copy information from Environment<int>
			{
				...
				err = clGetDeviceIDs(temp_platforms[platform_ind],
#ifdef SKEPU_SIMULATE_MULTI_GPU
//					CL_DEVICE_TYPE_CPU |
#endif
					CL_DEVICE_TYPE_GPU | CL_DEVICE_TYPE_ACCELERATOR, numDevices, deviceList, &numDevices);
