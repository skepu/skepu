...
namespace skepu
{
	namespace backend
	{
		...
		::reduceSingle_CL(size_t deviceID, size_t size, T &res, Iterator arg)
		{
...
#ifdef SKEPU_TRACING
			outMemP.m_label = "Internal (reduce-temp)";
			outMemP.m_container_id = UniqueIdentifier::generate();
#endif
			...
			auto scalar_res = Scalar<typename ReduceFunc::Ret>(res);
			SKEPU_TRACE_CALL(trace_handle, "Reduce", this, {size}, tracing::scalar_labels(scalar_res), {arg.getParent().getObjectID()}, {}, {});
			return scalar_res;
		}
