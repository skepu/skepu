/*! \file mapoverlap_4d_cu.inl
 *  \brief Contains the definitions of CUDA specific member functions for the MapOverlap4D skeleton.
 */
...
namespace skepu
{
	namespace backend
	{
		...
		/*!
		 *  Performs the 4D MapOverlap using a single CUDA GPU.
		...
		::mapOverlapSingleThread_CU(size_t deviceID, Parity p, pack_indices<OI...>, pack_indices<EI...>, pack_indices<AI...>, pack_indices<CI...>, CallArgs&&... args)
		{
			...
			auto in_mem_p = arg.updateDevice_CU(arg.getAddress(), in_size_i * in_size_j * in_size_k * in_size_l, deviceID, AccessMode::Read, true);
			auto out_mem_p = std::make_tuple(get<OI>(std::forward<CallArgs>(args)...).updateDevice_CU(get<OI>(std::forward<CallArgs>(args)...).getAddress(), out_size_i * out_size_j * out_size_k * out_size_l, deviceID, AccessMode::Write, true)...);
			...
			numThreads.x = std::min<size_t>(out_size_k, std::min<size_t>(maxThreads / numThreads_0a, sizeLength)) * numThreads_0a;
			...
			numBlocks.x = (out_size_k * out_size_l + numThreads.x - 1) / numThreads.x;
			...
			size_t sharedK = std::min<size_t>(in_size_k, numThreads.x / numThreads_0a) + this->m_overlap[2] * 2;
			size_t sharedJ = std::min<size_t>(in_size_j, numThreads.y) + this->m_overlap[1] * 2;
			size_t sharedI = std::min<size_t>(in_size_i, numThreads.z) + this->m_overlap[0] * 2;
			...
			if (this->isPool)
			{
				...
				sharedK = std::min<size_t>(out_size_k, numThreads.x / numThreads_0a) * this->m_overlap[2];
				sharedJ = std::min<size_t>(out_size_j, numThreads.y) * this->m_overlap[1];
				sharedI = std::min<size_t>(out_size_i, numThreads.z) * this->m_overlap[0];
			...
			// PRNG support
			size_t prng_threads = std::min<size_t>(out_size_i * out_size_j * out_size_k * out_size_l, numBlocks.x * numBlocks.y * numBlocks.z * numThreads.x * numThreads.y * numThreads.z);
			auto random = this->template prepareRandom<MapOverlapFunc::randomCount>(out_size_i * out_size_j * out_size_k * out_size_l, prng_threads);
			auto randomMemP = random.updateDevice_CU(random.getAddress(), prng_threads, deviceID, AccessMode::ReadWrite);
			
			
			DEBUG_TEXT_LEVEL1("CUDA " << name << " kernel: size = " << out_size_i * out_size_j * out_size_k * out_size_l << "x" << sizeof(T) << ", one device, "
							  << "numBlocks = [" << numBlocks.x << "x" << numBlocks.y << "x" << numBlocks.z << "], numThreads = [" << numThreads.x << "x" << numThreads.y << "x" << numThreads.z << "], "
							  << "shmem = " << sharedMemSize);
		...
		/*!
		*  Performs the 4D MapOverlap using multiple CUDA GPUs.
		*  The actual filter is specified in a user-function.
		 */
		template<typename MapOverlapFunc, typename CUDAKernel, typename CLKernel>
		template<size_t... OI, size_t... EI, size_t... AI, size_t... CI, typename... CallArgs>
		...
		::helper_CUDA(Parity p, pack_indices<OI...> oi, pack_indices<EI...> ei, pack_indices<AI...> ai, pack_indices<CI...> ci, CallArgs&&... args)
		{
			auto &res = get<0>(std::forward<CallArgs>(args)...);
			auto &arg = get<OutArity>(std::forward<CallArgs>(args)...);
			DEBUG_TEXT_LEVEL1("CUDA MapOverlap 4D: size = " << arg.size() << ", maxDevices = " << this->m_selected_spec->devices()
