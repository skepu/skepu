	...
	namespace backend
	{
		...
		*  The actual filter is specified in a user-function.
		...
		::mapOverlapSingleThread_CU(size_t deviceID, Parity p, pack_indices<OI...>, pack_indices<EI...>, pack_indices<AI...>, pack_indices<CI...>, CallArgs&&... args)
		{
			...
			size_t sharedK = std::min<size_t>(in_size_k, numThreads.x / numThreads_0a) + this->m_overlap_k * 2;
			size_t sharedJ = std::min<size_t>(in_size_j, numThreads.y) + this->m_overlap_j * 2;
			size_t sharedI = std::min<size_t>(in_size_i, numThreads.z) + this->m_overlap_i * 2;
			...
			if (isPool)
			{
				...
				sharedK = std::min<size_t>(out_size_k, numThreads.x / numThreads_0a) * this->m_overlap_k;
				sharedJ = std::min<size_t>(out_size_j, numThreads.y) * this->m_overlap_j;
				sharedI = std::min<size_t>(out_size_i, numThreads.z) * this->m_overlap_i;
			...
			(
				...
				numThreads_0a,
				(int)(isPool ? 1 : 0), this->m_edge, this->m_pad
			);
