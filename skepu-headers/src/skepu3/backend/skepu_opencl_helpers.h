...
namespace skepu { namespace backend { namespace cl_helpers {
inline std::string printCLError(int err);
}}}

#define CL_CHECK_ERROR(err, skepu_macro_text) {\
	if (err != CL_SUCCESS) {\
		std::cerr << "[" << debug_timestamp() << "][SKEPU_CL_ERROR " << __FILE__ << ":" << __LINE__ << "] " << skepu_macro_text << ": " << skepu::backend::cl_helpers::printCLError(err) << "\n"; \
		exit(1);\
	}\
}
...
#	define FPRINTF(a) fprintf a
			 ...
			 */
			inline std::string printCLError(cl_int Err)
			{
				std::string msg;
				if (Err != CL_SUCCESS)
				{
