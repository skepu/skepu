...
struct return_type_index
{
	
};

template<typename T>
struct return_type_index<T, 0>
{
	using type = T;
};

template<size_t I, typename T, typename... Ts>
struct return_type_index<typename skepu::multiple<T, Ts...>, I>
{
	using type = typename return_type_index<
		typename skepu::multiple<Ts...>,
		I - 1>::type;
};

template<typename T, typename... Ts>
struct return_type_index<typename skepu::multiple<T, Ts...>, 0>
{
	using type = T;
};
...
template<typename T>
struct memfun_type_sp {};

template<typename Ret, typename Class, typename... Args>
struct memfun_type_sp<Ret(Class::*)(Args...) const>
{
  using type = Ret(*)(Args...);
};

// Cast a captureless lambda to function pointer
template<typename F>
typename memfun_type_sp<decltype(&F::operator())>::type
lambda_cast_sp(F const& func)
{
	return (typename memfun_type_sp<decltype(&F::operator())>::type)func;
}
